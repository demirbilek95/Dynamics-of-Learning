...
class Optimizer:
    ...
    def updateParameters(self, t, w1, w2, w1_grads, w2_grads, momentum, nesterov_momentum, weight_decay):
        ...
        beta2 = 0.999
        if self.method == "SGD":
            w1, w2 = self.__SGD(w1, w2, w1_grads, w2_grads, weight_decay, momentum, nesterov_momentum)
        elif self.method == "Adagrad":
            w1, w2 = self.__Adagrad(w1, w2, w1_grads, w2_grads, weight_decay)
        elif self.method == "Adadelta":
            w1, w2 = self.__Adadelta(w1, w2, w1_grads, w2_grads, weight_decay, gamma)
        elif self.method == "RMSProp":
            w1, w2 = self.__RMSProp(w1, w2, w1_grads, w2_grads, weight_decay, gamma)
        elif self.method == "Adam":
            w1, w2 = self.__Adam(t, w1, w2, w1_grads, w2_grads, weight_decay, gamma, beta2)
        else:
            raise NotImplementedError

        return w1, w2

    def __SGD(self, w1, w2, w1_grads, w2_grads, weight_decay, momentum, nesterov_momentum):
        if weight_decay:
        ...
        if momentum:
            ...
            # TODO: This implementation is wrong, however they are not used in the experiments with momentum
    ...
    def __Adagrad(self, w1, w2, w1_grads, w2_grads, weight_decay):
        if weight_decay:
            w1_grads += weight_decay * w1
            w2_grads += weight_decay * w2
    ...
    def __Adadelta(self, w1, w2, w1_grads, w2_grads, weight_decay, gamma):
        if weight_decay:
            w1_grads += weight_decay * w1
            w2_grads += weight_decay * w2
    ...
    def __RMSProp(self, w1, w2, w1_grads, w2_grads, weight_decay, gamma ):
        if weight_decay:
            w1_grads += weight_decay * w1
            w2_grads += weight_decay * w2
        self.A1 = gamma * self.A1 + (1-gamma) * torch.square(w1_grads)
    ...
    def __Adam(self, t, w1, w2, w1_grads, w2_grads, weight_decay, beta1, beta2):
        if weight_decay:
            w1_grads += weight_decay * w1
            w2_grads += weight_decay * w2
